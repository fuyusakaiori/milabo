...
// 内部节点的头信息大小
const uint32_t INTERNAL_NODE_HEADER_SIZE = COMMON_TREE_NODE_HEADER_SIZE +
...
// 获取内部节点的关键字
uint32_t* internal_node_cell(void* node, uint32_t cell_num) {
    return node + INTERNAL_NODE_HEADER_SIZE + cell_num * TREE_INTERNAL_NODE_CELL_SIZE;
}

// 获取内部节点的关键字的 key
uint32_t* internal_node_key(void* node, uint32_t key_num) {
    // 注: 优先存放的是指针而不是 key, 为什么要这么存呢?
    return internal_node_cell(node, key_num) + TREE_INTERNAL_NODE_CHILD_SIZE;
}
